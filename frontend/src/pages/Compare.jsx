...
function getBest(schools, key) {
  ...
  const higherIsBetter = ["graduation_rate", "retention_rate", "median_earnings", "pct_earning_more_than_hs", "enrollment", "fulltime_pct"];
  const lowerIsBetter  = ["net_annual_cost", "in_state_tuition", "out_state_tuition", "median_debt", "monthly_loan_payment", "loan_default_rate", "acceptance_rate", "student_faculty_ratio"];

  if (higherIsBetter.includes(key)) return valid.reduce((a, b) => a.val > b.val ? a : b).id;
  if (lowerIsBetter.includes(key))  return valid.reduce((a, b) => a.val < b.val ? a : b).id;
  return null;
}
...
export default function Compare() {
  ...
  useEffect(() => { loadData(); }, [searchParams.get("ids")]);

  if (loading) return <div style={{ height: "calc(100vh - 58px)" }}><LoadingSpinner message="Loading comparison data..." /></div>;
  if (error)   return <div style={{ height: "calc(100vh - 58px)" }}><ErrorMessage message={error} onRetry={loadData} /></div>;

  if (ids.length === 0) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "calc(100vh - 58px)", flexDirection: "column", gap: "1rem", color: "var(--color-text-muted)", padding: "2rem", textAlign: "center" }}>
      ...
      <button onClick={() => navigate("/")} style={{ background: "var(--color-gold)", color: "var(--color-dark)", border: "none", borderRadius: "var(--radius-sm)", padding: "10px 24px", fontSize: "14px", fontWeight: "500", cursor: "pointer" }}>
        Back to Home
      </button>
    </div>
  );
  ...
  return (
    <div style={{ background: "var(--color-bg)", minHeight: "calc(100vh - 58px)" }}>
      <div style={{ background: "var(--color-dark)", padding: isMobile ? "1rem" : "1.5rem 2rem" }}>
        ...
        <h1 style={{ color: "#F1F5F9", fontSize: isMobile ? "17px" : "20px", fontWeight: "500", marginBottom: "4px" }}>
      ...
      <div style={{ padding: isMobile ? "1rem" : "1.5rem 2rem" }}>
        ...
        {/* School cards — stack on mobile */}
        <div style={{
          display:             "grid",
          gridTemplateColumns: isMobile ? "1fr" : `200px ${schools.map(() => "1fr").join(" ")}`,
          gap:                 "1rem",
          marginBottom:        "1.5rem",
        }}>
          {!isMobile && <div />}
            ...
            return (
              <div key={school.college_id} style={{ background: "var(--color-white)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", padding: "1.25rem", position: "relative" }}>
                <button
                  ...
                  style={{ position: "absolute", top: "10px", right: "10px", background: "transparent", border: "none", color: "var(--color-text-light)", fontSize: "18px", cursor: "pointer" }}
                >×</button>
                <div style={{ fontSize: "14px", fontWeight: "500", color: "var(--color-text)", marginBottom: "6px", paddingRight: "20px" }}>{school.college_name}</div>
                <span style={{ background: badge.bg, color: badge.color, fontSize: "10px", padding: "2px 8px", borderRadius: "4px", fontWeight: "500" }}>{school.category}</span>
                <div style={{ color: "var(--color-text-muted)", fontSize: "12px", marginTop: "8px" }}>{school.city}, {school.state}</div>
                {school.website && (
                  <a href={school.website} target="_blank" rel="noreferrer" style={{ color: "var(--color-gold)", fontSize: "11px", display: "block", marginTop: "6px" }}>Visit website ↗</a>
                )}
                <button onClick={() => navigate(`/school/${school.college_id}`)}
        ...
        {schools.length < 3 && (
          <div style={{ marginBottom: "1.5rem" }}>
            <button onClick={() => navigate("/")} style={{ background: "var(--color-white)", border: "2px dashed var(--color-border)", borderRadius: "var(--radius-md)", padding: "0.85rem 1.5rem", fontSize: "13px", color: "var(--color-text-muted)", cursor: "pointer", width: isMobile ? "100%" : "auto" }}>
              + Add another school
            </button>
          </div>
        )}
        ...
        {SECTIONS.map((section) => (
          <div key={section.title} style={{ background: "var(--color-white)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", marginBottom: "1.25rem", overflow: "hidden" }}>
            <div style={{ background: "#F8FAFC", padding: "0.85rem 1.25rem", borderBottom: "1px solid var(--color-border)", fontWeight: "500", fontSize: "12px", color: "var(--color-text)", textTransform: "uppercase", letterSpacing: "0.5px" }}>
              {section.title}
            </div>
            ...
            {section.rows.map((row, rowIdx) => {
              ...
              if (isMobile) {
                // Mobile: show label then each school value stacked
                return (
                  <div key={row.key} style={{ borderBottom: rowIdx < section.rows.length - 1 ? "1px solid #F1F5F9" : "none", padding: "0.85rem 1.25rem" }}>
                    <div style={{ fontSize: "12px", color: "var(--color-text-muted)", marginBottom: "0.5rem", fontWeight: "500" }}>{row.label}</div>
                    <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                      {schools.map((school) => {
                        const val    = getVal(school, row.key);
                        const isBest = bestId === school.college_id;
                        return (
                          <div key={school.college_id} style={{ background: isBest ? "#F0FDF4" : "#F8FAFC", borderRadius: "var(--radius-sm)", padding: "0.5rem 0.75rem", flex: "1", minWidth: "80px", textAlign: "center" }}>
                            <div style={{ fontSize: "10px", color: "var(--color-text-muted)", marginBottom: "3px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                              {school.college_name.split(" ").slice(0, 2).join(" ")}
                            </div>
                            <div style={{ fontSize: "14px", fontWeight: isBest ? "600" : "400", color: val == null ? "var(--color-text-light)" : isBest ? "#16A34A" : "var(--color-text)" }}>
                              {display(val, row.type)}
                            </div>
                            {isBest && val != null && (
                              <div style={{ fontSize: "9px", background: "#16A34A", color: "#fff", borderRadius: "3px", padding: "1px 5px", marginTop: "3px", display: "inline-block" }}>Best</div>
                            )}
                          </div>
                        );
                      })}
                  ...
                  {schools.map((school) => {
                    ...
                    return (
                      <div key={school.college_id} style={{ padding: "0.9rem 1.25rem", textAlign: "center", borderRight: "1px solid #F1F5F9", background: isBest ? "#F0FDF4" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
                        ...
                        {isBest && val != null && (
                          <span style={{ fontSize: "11px", background: "#16A34A", color: "#fff", borderRadius: "4px", padding: "1px 6px", fontWeight: "500" }}>Best</span>
                        )}
                      </div>
                    );
                  })}
        ...
        <div style={{ background: "var(--color-white)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-sm)", padding: "0.85rem 1.25rem", fontSize: "12px", color: "var(--color-text-muted)", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          ...
          <span>Indicates the best value among selected schools. For costs lower is better. For graduation rate and earnings higher is better.</span>
        </div>
      </div>
    </div>
  );
